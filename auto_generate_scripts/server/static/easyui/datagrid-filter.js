(function($){
	...
	$.fn.treegrid.methods.append = function(jq, param){
		return jq.each(function(){
			...
			if (opts.oldLoadFilter){
				...
				oldAppendMethod2($(this), param);
			}
		});
	};
	var oldInsertMethod2 = $.fn.treegrid.methods.insert;
	$.fn.treegrid.methods.insert = function(jq, param){
		return jq.each(function(){
			var state = $(this).data('treegrid');
			var opts = state.options;
			if (opts.oldLoadFilter){
